...
import vendor.mphx.connection.impl.Connection;
import vendor.mphx.connection.impl.WebsocketProtocol;
...
class Server implements IServer
{
	...
	public var maximumBufferSize = 6200000;
	 ...
	 * @param	_serializer
	 * @param	buffer : set the max read buffer (default = 16Kb) the buffer size is set with buffer * 1024
	 */
	public function new(hostname:String, port:Int, connectionTemplate : IConnection = null, _serializer : ISerializer = null, bufferSize : Int = 2048)
	{
		if (hostname == null)
			...
			if (socket == listener)
			{
				...
				while (true) //Reaching the end of the message or an error will break the loop
				{
					if (bytesReceived == len - 1){
						...
						buffer = Bytes.alloc(len + 2048); //Add an extra 1024 bytes space.
