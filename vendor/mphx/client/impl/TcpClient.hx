...
import vendor.mphx.connection.NetSock;
...
class TcpClient implements IClient
{
	...
	public function recieve(line:String)
	{
		...
		// trace is used here to outout the event names which will help in using the typeDef _trigger_event to determine the order the events are received from the server. The reason is that sometimes when sending two events one after another to the server, the server sends back the events in the wrong order. this trace can be used as a debug and fix that issue.
		if (msg.data._triggerEvent != "null") trace (msg.t);
	...
	public function dataReceived(input:Input):Void
	{
		...
		try{
			...
			loseConnection(vendor.mphx.utils.Error.ClientError.DroppedConnection);
			return;
		}
		recieve(line);
	}
	...
	public function loseConnection(?error:vendor.mphx.utils.Error.ClientError)
	{
