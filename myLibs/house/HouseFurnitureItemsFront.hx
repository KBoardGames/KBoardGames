...
class HouseFurnitureItemsFront extends FlxGroup
{
		...
		while (ii < RegHouse._totalPurchased + 1)
		{
			for (i in 0... RegHouse._totalPurchased+1)
			{
				if (RegHouse._item_order[i] == ii)
				{
					...
					_sprite.loadGraphic("myLibs/house/assets/images/house/furniture/put/" + (RegHouse._sprite_number[ii]) + ".png", true, RegHouse._item_animation_width[RegHouse._sprite_number[ii]], RegHouse._item_animation_height[RegHouse._sprite_number[ii]]);
	...
	{
		_sprite = new FlxSprite();
		_sprite.loadGraphic("myLibs/house/assets/images/house/furniture/put/" + HouseFurnitureGet._idCurrentItemPurchased + ".png", true, RegHouse._item_animation_width[HouseFurnitureGet._idCurrentItemPurchased], RegHouse._item_animation_height[HouseFurnitureGet._idCurrentItemPurchased]);
